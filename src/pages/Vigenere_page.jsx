...
const VigenerePage = () => {
  ...
  const processText = async () => {
    ...
    try {
      const payload = {
        text: userText,
        key: encryptionKey.trim(), // Use the string key for Vigenère cipher
      };

      const endpoint = mode === 'Encrypt' ? '/vigenere-encrypt' : '/vigenere-decrypt'; // Choose endpoint based on mode
      ...
      >
        <div className="container flex justify-center items-center mt-12 space-x-12">
          <div className="contactForm w-1/2 p-10 bg-gray-800 bg-opacity-80 flex flex-col space-y-6 rounded-lg shadow-xl">
            <h2 className="text-2xl text-gray-100 font-medium mb-6">Vigenère Cipher</h2>
